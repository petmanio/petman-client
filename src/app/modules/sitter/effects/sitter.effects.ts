...
export class SitterEffects {
  ...
  @Effect()
  create$ = this.actions$
    .ofType(SitterActionTypes.CREATE)
    .pipe(
      map((action: Create) => action.payload),
      switchMap(sitter => {
        return this.sitterService.create(sitter)
          .pipe(
  ...
  createSuccess$ = this.actions$
    .ofType(SitterActionTypes.CREATE_SUCCESS)
    .pipe(
  ...
  @Effect()
  update$ = this.actions$
    .ofType(SitterActionTypes.UPDATE)
    .pipe(
      map((action: Update) => action.payload),
      switchMap(({ id, body }) => {
        return this.sitterService.update(id, body)
          .pipe(
  ...
  updateSuccess$ = this.actions$
    .ofType(SitterActionTypes.UPDATE_SUCCESS)
    .pipe(
      map((action: CreateSuccess) => action.payload),
      tap(sitter => this.router.navigate(['sitters', sitter.id]))
    );
  ...
  @Effect()
  delete$ = this.actions$
    .ofType(SitterActionTypes.DELETE)
    .pipe(
      map((action: Delete) => action.payload),
      switchMap(id => {
        return this.sitterService.delete(id)
          .pipe(
  ...
  deleteSuccess$ = this.actions$
    .ofType(SitterActionTypes.DELETE_SUCCESS)
    .pipe(
      map((action: CreateSuccess) => action.payload),
  ...
  @Effect()
  list$ = this.actions$
    .ofType(SitterActionTypes.LIST)
    .pipe(
      map((action: List) => action.payload),
      switchMap(query => {
        return this.sitterService.list(query)
          .pipe(
  ...
  constructor(private router: Router, private actions$: Actions, private sitterService: SitterService) {
  }
}
